...
- 工作表结构合并拦截提示支持中英文；错误已显示时切换语言也会即时更新提示。
- 修复语义合并后 SVN 更新重新写入冲突标记的问题：更新 BASE 后恢复合并结果，再标记文本冲突已解决；更新失败保留恢复副本。
- “用最新”真正覆盖本地内容；“跳过”保留文件及 BASE。按文件检查远端状态，跳过后仍能发现待更新内容。
- XML 写回保留原始编码和 BOM，清理被替换的富文本子节点，并在覆盖前校验输出。
- 合并有效列取三方表头的并集；填补稀疏单元格时保持列顺序。远端工作表增删及删除/修改冲突明确阻止保存。
- 修复表头、文件名、Sheet 名等内容的 HTML/事件处理器注入，以及删除行后漏显示其他行修改的问题。
- 新增 XML/API、真实 SVN 临时仓库和前端渲染回归测试，并接入 CI。
